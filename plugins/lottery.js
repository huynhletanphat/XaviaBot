...
async function findTheWinners() {
  ...
  const allLotteryPlayers = Array.from(global.data.users.values()).filter(
    (e) => e.data.lottery && e.data.lottery.bet,
  );
  const playersWinBet = allLotteryPlayers
    .filter((e) => e.data.lottery.numberBet == randomNumber)
    .map((e) => {
      return e.userID;
    });
  const playersLoseBet = allLotteryPlayers.filter(
    (e) => !playersWinBet.includes(e.userID),
  );
