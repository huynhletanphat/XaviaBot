...
async function makeImage(data) {
  ...
  try {
    const template = await Canvas.loadImage(
      join(global.assetsPath, "love.png"),
    );

    const avatarOne = await Canvas.loadImage(avatarPathOne);
    const avatarTwo = await Canvas.loadImage(avatarPathTwo);

    const avatarOneCircle = await global.circle(
      avatarOne,
      avatarOne.width / 2,
      avatarOne.height / 2,
      avatarOne.width / 2,
    );
    const avatarTwoCircle = await global.circle(
      avatarTwo,
      avatarTwo.width / 2,
      avatarTwo.height / 2,
      avatarTwo.width / 2,
...
async function onCall({ message, getLang }) {
  ...
  try {
    ...
    avatarPathOne = join(
      global.cachePath,
      `rank_avatar_${senderID}_${Date.now()}.jpg`,
    );
    avatarPathTwo = join(
      global.cachePath,
      `rank_avatar_${targetID}_${Date.now()}.jpg`,
    );
    ...
    if (!result) message.reply("Error");
      ...
      await message.reply({
        body: getLang("loveMessage"),
        attachment: global.reader(savePath),
      });
  } catch (e) {
    console.error(e);
    message.reply("Error");
