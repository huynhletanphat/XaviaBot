...
async function getVideoInfo(id) {
  try {
    const { data } = await global.GET(
      `${global.xva_api.main}/ytvideodetails?id=${id}`,
    );
...
async function searchByKeyword(keyword, MAX_VIDEOS) {
  try {
    if (!keyword) return [];
    const { data } = await global.GET(
      `${global.xva_api.main}/ytsearch?keyword=${encodeURIComponent(keyword)}&maxResults=${MAX_VIDEOS}`,
    );
...
async function onCall({ message, args, extra, getLang }) {
  try {
    ...
    if (!url.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
      ...
      const thumbnails = await downloadThumbnails(
        items.map((item) => item.snippet.thumbnails.high.url),
      );

      attachments.push(
        ...(thumbnails || []).map((path) => global.reader(path)),
      );
